...
[官网](https://multica.ai) · [云服务](https://multica.ai/app) · [X](https://x.com/multica_hq) · [部署指南](DEPLOYING.md) · [自部署指南](SELF_HOSTING.md) · [参与贡献](CONTRIBUTING.md)
...
不再需要复制粘贴 prompt，不再需要盯着运行过程。你的 Agent 出现在看板上、参与对话、随着时间积累可复用的技能。可以理解为开源的 Managed Agents 基础设施——厂商中立、可自部署、专为人类 + AI 团队设计。支持 **Claude Code**、**Codex**、**OpenClaw**、**OpenCode** 和 **Gemma**（通过 Google AI API）。
...
daemon 在后台运行，保持你的机器与 Multica 的连接。它会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`、`openclaw`、`opencode`）。设置 `MULTICA_GEMMA_API_KEY` 还可启用 Gemma 后端（无需安装 CLI）。
...
进入 **设置 → Agents**，点击 **新建 Agent**。选择你刚连接的 Runtime，选择 Provider（Claude Code、Codex、OpenClaw、OpenCode 或 Gemma），并为 Agent 起个名字——它将以这个名字出现在看板、评论和任务分配中。
...
| Agent 运行时 | 本地 daemon 执行 Claude Code、Codex、OpenClaw、OpenCode 或 Gemma |

## 部署

Multica 内置 GitHub Actions 流水线，每次推送到 `main` 分支后自动部署：

| 层级 | 平台 |
|------|------|
| 前端 | [Vercel](https://vercel.com) |
| 后端 API | [Render](https://render.com) |
| 数据库 | Render 托管 PostgreSQL |

每次后端部署时，数据库迁移会自动运行。详细配置步骤、所需 Secrets 及环境变量，请参阅 [部署指南](DEPLOYING.md)。

> **想在自己的机器或服务器上部署？** 请参阅 [自部署指南](SELF_HOSTING.md)。
